...
class SQLContext(@transient val sparkContext: SparkContext)
  ...
  protected[sql] def doPriCheck(logicalPlan: LogicalPlan,authMode: String): Unit = Nil
  ...
  protected[sql] class QueryExecution(val logical: LogicalPlan) {
    ...
    val authorized: LogicalPlan = {
      logInfo("SparkAuthDebug : doing privilege check in QueryExecution")
//      var authMode:String = "V2"
//      val confAuthMode = conf.getAllConfs.get("hive.spark.authorization.mode").get
//      if(confAuthMode!=null){
//          authMode = confAuthMode
//      }
//      doPriCheck(analyzed,authMode)
      doPriCheck(analyzed,"V2")
