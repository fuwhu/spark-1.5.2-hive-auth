...
import org.apache.spark.sql.hive.acl.{RedisWhiteList, HiveACLConstants}
...
private[hive] class HiveMetastoreCatalog(val client: ClientInterface, hive: HiveContext)
  ...
  def lookupRelation(
    ...
    /** For authorization **/
    val confMap : Map[String,String] = conf.getAllConfs

    var whiteListEnabled:Boolean = false
    if(confMap.contains("hive.spark.acl.whitelist.enable")){ whiteListEnabled = confMap.get("hive.spark.acl.whitelist.enable").get.equals("true")}
    logInfo("hive.spark.acl.whitelist.enable is " + whiteListEnabled)

    if(whiteListEnabled){
      if(confMap.contains("user.name")){ logInfo("user.name from hive.conf is "+confMap.get("user.name")) }
      else{ logInfo("no user.name in hive.conf") }

      if(System.getProperty("user.name")!=null){ logInfo("user.name from system property is "+System.getProperty("user.name")) }
      else{ logInfo("no user.name in system property") }

      if(System.getenv("USER")!=null){ logInfo("USER from system env is "+System.getenv("USER")) }
      else{ logInfo("no USER in system env") }

      var host:String = HiveACLConstants.DefaultHiveAclRedisHost
      if(confMap.contains("hive.spark.acl.whitelist.redis.host")){ host = confMap.get("hive.spark.acl.whitelist.redis.host").get }
      logInfo("hive.spark.acl.whitelist.redis.host is " + host)

      var port:Int = HiveACLConstants.DefaultHiveAclRedisPort
      if(confMap.contains("hive.spark.acl.whitelist.redis.port")){ port = confMap.get("hive.spark.acl.whitelist.redis.port").get.toInt }
      logInfo("hive.spark.acl.whitelist.redis.port is " + port)

      var db:Int = HiveACLConstants.DefaultHiveACLRedisDB
      if(confMap.contains("hive.spark.acl.whitelist.redis.db")){ db = confMap.get("hive.spark.acl.whitelist.redis.db").get.toInt }
      logInfo("hive.spark.acl.whitelist.redis.db is " + db)

      var keyPrefix:String = HiveACLConstants.DefaultHiveAclWhiteListKeyPrefix
      if(confMap.contains("hive.spark.acl.whitelist.redis.keyprefix")){ keyPrefix = confMap.get("hive.spark.acl.whitelist.redis.keyprefix").get }
      logInfo("hive.spark.acl.whitelist.redis.keyprefix is " + keyPrefix)

      val isPermitted = RedisWhiteList.accessPermitted(host,port,db,keyPrefix,databaseName,tblName)
      if(!isPermitted){
        throw new HiveAccessControlException("Not permitted to access "+databaseName+"."+tblName)
      }
    }
