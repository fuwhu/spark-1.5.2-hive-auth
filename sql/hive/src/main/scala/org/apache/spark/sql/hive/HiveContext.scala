...
class HiveContext(sc: SparkContext) extends SQLContext(sc) with Logging {
  ...
  override protected[sql] def doPriCheck(logicalPlan: LogicalPlan,authMode: String): Unit = {
    logInfo("SparkAuthDebug : doing Privilege Check in HiveContext")
    ...
    if(authMode.equals("V1"))
        metadataHive.checkPrivilegesV1(logicalPlan)
    else
        metadataHive.checkPrivilegesV2(logicalPlan)
